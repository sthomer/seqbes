...
import Data.List (elemIndex, inits, intercalate, intersect, nub, sort, sortOn, tails, union, (\\), foldl', permutations)
...
mkJoint :: Int -> Int -> Joint String
mkJoint n k = M.fromList $ zip (map Context contexts) probs
  where
    states = map (Token . (: [])) $ take n ['a'..'z']
    contexts = (nub . map (take k) . permutations . concatMap (replicate k)) states
    weights = take (length contexts) $ randoms (mkStdGen k)
    total = sum weights
    probs = map (Probability . (/ total)) weights

margin :: (Eq a, Hashable a) => Int -> Joint a -> Joint a
margin n joint = M.fromListWith (+) $ map removeAt $ M.toList joint
  where
    removeAt (Context k, v) = (Context $ deleteAt (n-1) k, v)

--marginal :: [Int] -> ([Int], Joint a) -> ([Int], Joint a)
--marginal xs (ys, joint) = undefined
--  where
--    xs' = ys \\ xs

--termWithJoint :: EntropyTerm -> Joint a -> Joint a
--termWithJoint (EntropyTerm (s, y, xs)) joint = margin
--  where
--    jointLength = (length . unContext . fst . head . M.toList) joint
--    (_, margin) = marginal xs ([1..jointLength], joint)
